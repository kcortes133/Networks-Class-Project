...
from embiggen.embedders.ensmallen_embedders import DeepWalkGloVeEnsmallen
...
def graphEmbedding(g):
    ...
    plt.hist(simRange)
    plt.show()
    rankedGenes = sorted(simGenes.items(), key=lambda item: item[1], reverse=True)
    for gene in rankedGenes[:5]:
        print(gene)
        if 'HGNC' in gene[0]:
            print(gene[0])

    #et = EdgeTransformer(method='CosineSimilarity', aligned_mapping=True).fit_transform(embeddingDF)
    #print(embeddingDF)
    #print(et)
    #cosSims = et.transform(sources=embeddingDF.sources.to_numpy(), destinations=embeddingDF.destinations.to_numpy())
    #plt.hist(cosSims)
    #plt.show()
